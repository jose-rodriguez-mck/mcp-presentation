...
2. **Check if template exists**:
   - Load from `.sdd/settings/templates/steering-custom/{name}.md` if available
...
4. **Generate custom steering**:
   ...
   - Apply principles from `.sdd/settings/rules/steering-principles.md`
   - Focus on patterns, not exhaustive lists
   - Keep to 100-200 lines (2-3 minute read)

5. **Create file** in `.sdd/steering/{name}.md`

## Available Templates

Templates available in `.sdd/settings/templates/steering-custom/`:
